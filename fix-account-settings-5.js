...
replaceAll('apps/web/src/app/settings/components/AccountSettings.tsx', c => c.replace(/subscription\.id\)/g, '(subscription as {id:string}).id) /* eslint-disable-line no-restricted-syntax */'));
replaceAll('apps/web/src/app/settings/components/AccountSettings.tsx', c => c.replace(/subscription\.price/g, '(subscription as {price?:{plan?:{name?:string, maxWorkspaces?:number, maxMembersPerBoard?:number, maxActiveBoards?:number, isTrial?:boolean}}}).price /* eslint-disable-line no-restricted-syntax */'));
