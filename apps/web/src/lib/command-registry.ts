    ...
    action: ({ args, printOutput, virtualPath }) => {
      ...
      import("@syncoboard/api").then(({ directoryApi }) => {
        directoryApi
          ...
          .catch((err: unknown) => {
            const errorMessage =
              (err && typeof err === "object" && "response" in err ? (err as {response?:{data?:{error?:string}}}).response?.data?.error : undefined) ||
               (err instanceof Error ? (err instanceof Error ? err.message : "") : "") ||
    ...
    action: ({ args, printOutput, activeBoardId, setMode }) => {
      ...
      import("./actions/tasks").then(({ addTask }) => {
        addTask(activeBoardId, title)
          ...
          .catch((err: unknown) => {
            const errorMessage =
              (err instanceof Error ? (err instanceof Error ? err.message : "") : "") || "Failed to add task.";
            printOutput([`Error: ${errorMessage}`]);
          });
      });
    },
    ...
    action: ({ args, printOutput, setMode }) => {
      ...
      import("./actions/tasks").then(({ updateTaskStatus }) => {
        updateTaskStatus(taskId, status)
          ...
          .catch((err: unknown) => {
            const errorMessage =
              (err instanceof Error ? (err instanceof Error ? err.message : "") : "") || "Failed to update task status.";
            printOutput([`Error: ${errorMessage}`]);
          });
      });
    },
    ...
    action: ({ args, printOutput, setMode }) => {
      ...
      import("./actions/tasks").then(({ deleteTask }) => {
        deleteTask(taskId)
          ...
          .catch((err: unknown) => {
            const errorMessage =
              (err instanceof Error ? (err instanceof Error ? err.message : "") : "") || "Failed to delete task.";
            printOutput([`Error: ${errorMessage}`]);
          });
      });
    },
