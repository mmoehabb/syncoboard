...
export class VoiceApi extends ApiClient {
  ...
  public async join(boardId: string, peerId: string): Promise<{ success: boolean }> {
    const response = await this.post<{ success: boolean }>("/join", { boardId, peerId });
    return response.data;
  }

  public async leave(boardId: string): Promise<{ success: boolean }> {
    const response = await this.post<{ success: boolean }>("/leave", { boardId });
    return response.data;
  }
  ...
  public async ping(boardId: string): Promise<{ success: boolean }> {
    const response = await this.post<{ success: boolean }>("/ping", { boardId });
    return response.data;
  }
}
