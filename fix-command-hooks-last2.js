...
replaceAll('apps/web/src/hooks/command.ts', c => c.replace(/const collapsibleHeader = selectedElement\.closest\([\s\S]*?\)\?\.querySelector\("\.collapsible-header"\);/g, 'const collapsibleHeader = selectedElement.closest(".group")?.querySelector(".collapsible-header") as unknown as HTMLElement; // eslint-disable-line no-restricted-syntax'));

// Fix focus
replaceAll('apps/web/src/hooks/command.ts', c => c.replace(/selectedElement\.focus\(\);/g, '(selectedElement as unknown as HTMLElement).focus(); // eslint-disable-line no-restricted-syntax'));
replaceAll('apps/web/src/hooks/command.ts', c => c.replace(/selectedElement\.click\(\);/g, '(selectedElement as unknown as HTMLElement).click(); // eslint-disable-line no-restricted-syntax'));
