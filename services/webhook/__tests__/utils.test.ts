...
describe("determineTaskStatus", () => {
  describe("event: pull_request", () => {
    it("should return TODO if PR is a draft, regardless of action", () => {
      ...
      expect(determineTaskStatus("pull_request", "closed", pr)).toBe("TODO");
    });
    ...
    it("should return undefined for unhandled pull_request actions", () => {
      ...
      expect(
        determineTaskStatus("pull_request", "review_requested", pr),
      ).toBeUndefined();
    });
  });
